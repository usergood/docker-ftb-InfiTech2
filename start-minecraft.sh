...
case "X$FTBPACK" in
  X[Hh][Tt][Tt][Pp]*[Zz][iI][pP])
	...
	mv /tmp/ftbpack/libraries /data
	mv /tmp/ftbpack/mods /data
	mv /tmp/ftbpack/scripts /data
	rm -rf /tmp/ftbpack
    rm -f /tmp/modpack.zip
    ;;
  *)
    echo "Invalid URL given for ftbpriv-pack: Must be HTTP or HTTPS and a ZIP file"
    ;;
esac
fi
